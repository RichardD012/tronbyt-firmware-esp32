...
static std::unique_ptr<MatrixPanel_I2S_DMA> _matrix;
...
int display_initialize() {
  ...
  _matrix = std::make_unique<MatrixPanel_I2S_DMA>(mxconfig);

  if (!_matrix) {  // Should not happen with make_unique if it throws
                   // std::bad_alloc
